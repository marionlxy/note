...
![img](http://img.liuwenqi.com/blog/2019-07-14-141453.jpg)
...
slow_query_log = 0.01 ##判断大于1s输出日志
long_query_time = 0.01 ##判断大于1s输出日志
...
```l
mysql日志结构：
# User@Host: zabbix[zabbix] @  [172.16.249.151]  Id: 11721
# Query_time: 1.368911  Lock_time: 0.000921 Rows_sent: 0  Rows_examined: 264177
SET timestamp=1573288112;
SELECT
  ...
  LIMIT 1;
# Time: 2019-11-09T10:20:23.565980Z
# User@Host: zabbix[zabbix] @  [172.16.249.151]  Id: 11721
# Query_time: 1.368911  Lock_time: 0.000921 Rows_sent: 0  Rows_examined: 264177
SET timestamp=1573288845;
SELECT
    table_name as table_name,
    ( SELECT
        column_name as column_name
...
filter {
  ...
  grok {
        match => ["message","(?m)^# User@Host: %{USER:user}\[[^\]]+\] @ (?:(?<clienthost>\S*) )?\[(?:%{IP:clientip})?\]  Id: %{NUMBER:thread_id}\s*# Query_time: %{NUMBER:query_time:float}\s+Lock_time: %{NUMBER:lock_time:float}\s+Rows_sent: %{NUMBER:rows_sent:int}\s+Rows_examined: %{NUMBER:rows_examined:int}\s*(?:use %{DATA:database};\s*)?SET timestamp=%{NUMBER:timestamp};\s*(?<query>(?<action>\w+)\s+.*)*$"]
        }
