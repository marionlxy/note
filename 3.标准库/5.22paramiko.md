...
```python
# 基于秘钥对儿连接远程服务器	    #windows环境下ssh-keygen -m PEM -t rsa  指定生成的密钥格式为PEM型
import paramiko             #使用ssh-copy-id 将本机公钥传送到远程主机
						    #windows下地址用windows格式，盘符前面加小r：(r'C:\')
...
```python
# 基于加密通道传输	       #windows环境下ssh-keygen -m PEM -t rsa  指定生成的密钥格式
import paramiko			#使用ssh-copy-id 将本机公钥传送到远程主机
						#windows下地址用windows格式，盘符前面加小r：(r'C:\')
private = 	paramiko.RSAKey.from_private_key_file('/Users/chaoliu/.ssh/id_rsa')
...
```python
# 基于加密通道进行上下传文件          #windows环境下ssh-keygen -m PEM -t rsa  指定生成的密钥格式
import paramiko                  #使用ssh-copy-id 将本机公钥传送到远程主机
                                 #windows下地址用windows格式，盘符前面加小r：(r'C:\')
private = paramiko.RSAKey.from_private_key_file('/Users/chaoliu/.ssh/id_rsa')
transport = paramiko.Transport(('39.100.110.135', 22))
